...
<script>
import { listInsert, listDetail, listUpdata } from '@/api/goods';
...
export default {
    ...
    methods: {
                ...
                if (valid) {
                    ...
                    if (this.imgArr1) {
                        ...
                        for (var i = 0; i < this.imgArr1.length; i++) {
                            str.push(this.imgArr1[i].url);
                        }

                        param.goodMainPhoto = str.join(',');
                    }
                    ...
                    if (this.imgArr2) {
                        ...
                        for (var i = 0; i < this.imgArr2.length; i++) {
                            str.push(this.imgArr2[i].url);
                        }

                        param.goodAppendedDrawings = str.join(',');
                    }

                    console.log('新增商品', this.imgArr2);
                    listUpdata(param).then(res => {
                        if (res.code == 200) {
                            this.$message({
                                message: '新增成功',
                                type: 'success'
        ...
        handleRemove1(file, fileList) {
            console.log('删除', file, fileList);
            for (var i = 0; i < this.imgArr2.length; i++) {
                if (this.imgArr2[i].uid == file.uid) {
